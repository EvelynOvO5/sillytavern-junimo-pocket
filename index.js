...
const defaults={enabled:false,proactive:true,url:'',model:'',maxChars:12000,history:16,maxTokens:2200,loreChars:18000,boundAvatar:'',left:null,top:70};
  ...
  try{
    ...
    for(let i=keep;i<rows.length;i++){
      ...
      if(!m.is_user&&!m.is_system&&m.mes?.trim()){
        ...
        const background=await roleContext('',String(m.mes).slice(-config().maxChars),controller.signal);if(run!==epoch)return;
        const input=JSON.stringify({roles:ui.roles,state,background,recentPhoneMessages:previews,preceding:rows.slice(Math.max(0,i-2),i).map(x=>String(x.message.mes).slice(-1500)),narrative:String(m.mes).slice(-config().maxChars)});
...
function openSettings(){
  ...
  const help=document.createElement('p');help.textContent='自动读取月亮谷角色卡绑定的世界书，人设无需手填。启用后会把相关世界书条目、正文和状态发送到此 API。Key 仅保留在本页，刷新后需重填。';settingsPanel.append(help);
  ...
  async function save(){endpoint(url.value.trim());if(!model.value.trim())throw Error('请填写模型');for(const [input,min,max] of [[chars,1000,40000],[history,0,60],[tokens,300,12000],[loreChars,2000,40000]])if(!Number.isInteger(+input.value)||+input.value<min||+input.value>max)throw Error('上下文或输出上限超出范围');invalidate();Object.assign(cfg,{url:url.value.trim(),model:model.value.trim(),enabled:enabled.checked,proactive:proactive.checked,maxChars:+chars.value,history:+history.value,maxTokens:+tokens.value,loreChars:+loreChars.value});delete cfg.persona;delete cfg.profiles;sessionKey=key.value.trim();context().saveSettingsDebounced();status('设置已保存');}
  ...
  button('同步当前正文 / 重试',async()=>{await save();await syncNarrative();});
