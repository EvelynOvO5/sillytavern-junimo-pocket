...
async function send(role,text=''){
 ...
 const response=await completion([{role:'system',content:chatInstruction(role,ui.roles)+'\n你是'+role.name+'，正在手机聊天App里和用户文字聊天。双方可能不在同一个地方。保持世界书人设，不写旁白、动作描写，不替用户说话；只能知道自己有理由知道的事情。阅读用户连续发来的全部气泡后一起回复。线上对话比较随意，根据角色人设和心情使用标点，不用每次结尾都加句号，可以直接不带结尾标点，也不强行删除表达情绪的问号或感叹号。像真人打字，可随意分成1至6条短消息，长短不一，不要每次一大段，不必机械问句收尾。严格使用上面规定的私聊或群聊返回格式。世界书资料：'+background+'\n当前正文：'+narrative},...messages],controller.signal);
